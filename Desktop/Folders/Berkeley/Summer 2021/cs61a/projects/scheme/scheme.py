...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, do_lambda_form(new_expression, env))
        return target.first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    result = True
    while expressions:
        if expressions.rest is nil: #new
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            ...
            if not clause.rest:
                return test
            return eval_all(clause.rest, env, True)
            # END PROBLEM 13
        expressions = expressions.rest
...
def do_let_form(expressions, env):
    ...
    return eval_all(expressions.rest, let_env, True)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    #var1 = Pair(x, Pair(5, nil)
    #var2 = Pair(y, Pair(4, nil)
    #var3 = Pair(y, Pair(4, nil)
    #bindings = Pair(var1, Pair(var2, Pair(var3, nil)))

    # names = Pair(bindings.first.first, Pair(bindings.first.first, nil))
    ...
    while bindings:
        validate_form(bindings.first, 2, 2)
        temp_names.rest = Pair(bindings.first.first, nil)
        temp_values.rest = Pair(scheme_eval(bindings.first.rest.first, env), nil)
        temp_names = temp_names.rest
        temp_values = temp_values.rest
        bindings = bindings.rest
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        formals = target.rest
        body = expressions.rest
        procedure_new = MacroProcedure(formals, body, env)
        env.define(macro_name, procedure_new)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
    ...
    while True:
        try:
            ...
            while src.more_on_line:
                ...
                result = scheme_eval(expression, env, False)
