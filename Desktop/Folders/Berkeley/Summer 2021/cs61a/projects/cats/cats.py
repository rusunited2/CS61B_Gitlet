...
def choose(paragraphs, select, k):
    ...
    for x in paragraphs:
        if select(x):
            valid_paragraph.append(x)
    return valid_paragraph[k] if k < len(valid_paragraph) else ''
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    if not typed_words:
        ...
        if typed_words:
            return 0.0
    for t in range(min(len(typed_words), len(reference_words))):
        if typed_words[t] == reference_words[t]:
                correct += 1
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        for word in valid_words:
            lst.append([word, diff_function(typed_word, word, limit)])
        if min(lst, key = lambda x: x[1])[1] <= limit:
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    # counter = abs(len(goal) - len(start))
...
def time_per_word(times_per_player, words):
    ...
    #         lst[i] += times_per_player[i][j + 1] - times_per_player[i][j]
    # print(lst)]

    times_list = []
    for j in range(len(times_per_player)):
        times = [times_per_player[j][i] - times_per_player[j][i-1] for i in range(1, len(words)+1)]
...
def fastest_words(game):
    ...
    #         lst.append(time(game, player, word_index))
    lst = []
    ...
    for word_index in word_indices:
        ...
        for player_num in player_indices:
            lst2.append(time(game, player_num, word_index))
        player_index = lst2.index(min(lst2))
        lst[player_index].append(word_at(game, word_index))
    return lst
    # END PROBLEM 10
