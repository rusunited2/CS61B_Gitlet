...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # if place.ant == ContainerAnt and self == Ant 
            # place.ant.store_ant --> self
            if place.ant.can_contain(self):
                place.ant.store_ant( self)

            # elif place.ant == Ant and self == ContainerAnt
            ...
            # place.ant = self
                ...
                assert place.ant == None, 'Two ants in {0}'.format(place)
            # END Problem 8
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        a, x = self.place, self.min_range
        for _ in range(self.min_range):
            if a != None:
                a = a.entrance
        while a != None and not a.is_hive and self.min_range <= x <= self.max_range:
            if a.bees:
                return choose_bee(a.bees)
            a, x = a.entrance, x + 1
        return None
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.chew_countdown > 0:
            ...
            if self.place.bees:
                random_bee = choose_bee(self.place.bees)
...
# END Problem EC
    ...
    def __init__(self, health=1):
        ...
        self.is_real_queen = len(self.queens_list) == 1
        # self.number = self.queen_num
        # self.queen_num += 1
        # END Problem EC
        ...
        if not self.is_real_queen:
            ...
            while curr_place:
                if curr_place.ant:
                    ...
                    #     if curr_place.ant.damage and not curr_place.ant.double_damage:
                    #         curr_place.ant.double_damage = True
                    #         curr_place.ant.damage *= 2
                    #     if curr_place.ant.ant_contained and not curr_place.ant.ant_contained.double_damage:
                    #         curr_place.ant.ant_contained.double_damage = True
                    #         curr_place.ant.ant_contained.damage *= 2
                        
                    # else:
                    #     if not curr_place.ant.double_damage:
                    #         curr_place.ant.double_damage = True
                    #         curr_place.ant.damage *= 2
                    if curr_place.ant.damage and not curr_place.ant.double_damage:
                        curr_place.ant.double_damage = True
                    ...
                    if curr_place.ant.is_container and curr_place.ant.ant_contained and not curr_place.ant.ant_contained.double_damage:
                        curr_place.ant.ant_contained.double_damage = True
                        curr_place.ant.ant_contained.damage *= 2
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem EC
        # self.health -= amount
        # if self.health <= 0 and self.is_real_queen:
        #     self.bees_win()
        # if self.is_real_queen:
        if self.is_real_queen:
            #use special reduce health
            self.health -= amount
            if self.health <= 0:
                bees_win()
        else:
